...
function FusionRecipe({ recipe, scale = 1, includeProduct = true }) {
    ...
    recipe.ingredients.forEach(ingredient => {
        if (components.length !== 0) components.push(<span key={components.length} style={fontStyle}>+</span>);
    ...
    if (includeProduct) {
        components.unshift(<span key={components.length} style={fontStyle}>=</span>);
        components.unshift(<Gift key={components.length} id={recipe.id} scale={scale} />);
    }
