...
function useStableSet(arr) {
  var ref = useRef(new Set(arr));
  var prev = ref.current;
  var next = new Set(arr);
  var changed = prev.size !== next.size || _toConsumableArray(next).some(function (x) {
    return !prev.has(x);
  });
  if (changed) {
    ref.current = next;
...
export function useDataMultiple() {
  var paths = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  var enabled = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  var stableSet = useStableSet(paths);
  var _useContext2 = useContext(DataContext),
    dataStore = _useContext2.dataStore,
    getData = _useContext2.getData;
  var _useState7 = useState({}),
    _useState8 = _slicedToArray(_useState7, 2),
    dataMap = _useState8[0],
    setDataMap = _useState8[1];
  var requestedRef = useRef(new Set());
  useEffect(function () {
    setDataMap(function () {
      var next = {};
      stableSet.forEach(function (p) {
        var _dataStore$p;
        next[p] = (_dataStore$p = dataStore[p]) !== null && _dataStore$p !== void 0 ? _dataStore$p : null;
      });
      return next;
    });
  }, [stableSet, dataStore]);
  useEffect(function () {
    if (!enabled) return;
    stableSet.forEach(function (path) {
  ...
  var loading = enabled && _toConsumableArray(stableSet).some(function (p) {
    return dataMap[p] == null;
  });
